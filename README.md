...
A **FastAPI** service that turns **natural-language questions** into **read-only Oracle SQL** through a **LangGraph** agent. The agent talks to Oracle via **SQLcl `-mcp` (stdio)** and exposes a chat UI at **`/gui/`** and a streaming JSON API at **`/api/v1/chat-stream/`**.
...
The agent has two parallel database paths and you keep them both: SQLcl-MCP gives the model rich introspection (table descriptions, sample rows, query execution), and the curated `tools.py` wrappers enforce a **strict read-only `SELECT`** guard with sqlglot-based cure-and-validate against `schema.json`.
...
| `src/nl2sql_agent/executor.py` | sqlglot SELECT-only guard, cure-and-validate, `oracledb` runner |
| `src/nl2sql_agent/oracle_catalog.py` | `ALL_OBJECTS` lookup used by `verify_identifier_in_catalog` |
...
- **Oracle read-only user** credentials used by `python-oracledb` and SQLcl
- **Java 17+** on `PATH` — required by SQLcl
...
| `SQLCL_PATH=vendor/sqlcl/bin/sql.exe` | `vendor/sqlcl/bin/sql` on macOS/Linux |
| `SQLCL_USER_DIR=.sqlcl` | Keeps SQLcl's wallet inside the repo (gitignored) |
| `SQLCL_CONNECTIONS=[main,RO_USER/RO_PASSWORD@host:1521/service]` | Saved on lifespan boot |
...
`SQLCL_PATH` gates the whole agent stack:

| `SQLCL_PATH` | Outcome |
|--------------|---------|
| Empty (default) | Lifespan logs a warning. `/api/v1/chat-stream/` returns **HTTP 503**. NiceGUI is **not** mounted. `/health` still works. |
...
| Set + invalid | Lifespan logs a warning and downgrades to "agent disabled" mode (same as empty). |
...
| `SQLCL_PATH` | Absolute path to SQLcl `sql`/`sql.exe`. **Empty disables the agent entirely.** |
